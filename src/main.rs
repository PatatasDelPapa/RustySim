...
fn run(receptor: mpsc::Receiver<(Command, oneshot::Sender<Response>)>) {
    tokio::spawn( system(receptor) );
}

async fn system(mut receptor: mpsc::Receiver<(Command, oneshot::Sender<Response>)>) {
    let mut lista_passivates = HashMap::new();
    let mut future_event_list = PriorityQueue::new();
    let mut future_event_list_aux = HashMap::new();
    let mut resultado;
    let mut tiempo: Time = 0;
                ...
                match command {
                    ...
                    Command::Activate(id) => {
                        ...
                        match resultado {
                            ...
                            None => {
                                ...
                                log::error!("id: {} no esta duermiendo", id);
                    ...
                    Command::Advance => {
                        ...
                        match some_id {
                            ...
                            None => {
                                log::warn!("Ninguna ID se encuentra presente en la FEL");
                                // Panic !?
                                response.send(Response::Continue).unwrap();
                            }
                        }
...
async fn advance(channel: &mut mpsc::Sender<(Command, oneshot::Sender<Response>)>) {
    loop {
        ...
        let resultado = channel.send((Command::Advance, tx)).await;
        match resultado {
            Ok(_) => {},
            Err(e) => { 
                log::error!("Ocurrio un error mandando advance. Error: {}", e);
                panic!("Mandar un Advance no deberia fallar");
            },
        };
...
// TODO: Agregarle estado al objeto
...
impl Pausable for Objeto {
    async fn hold(&mut self, t: u64) {
        loop {
            ...
            //     .ok()
            //     .unwrap();
            let resultado = self.channel.send((Command::Hold(self.id, t), tx)).await;
            match resultado {
                Ok(_) => {},
                Err(e) => {
                    log::error!("Mandar Hold con id: {} Ha fallado. Error: {}", self.id, e);
                    panic!("Mandar Hold no deberia fallar");
                }
            };
            let result = rx.await.unwrap();
            if let Response::Breaker = result {
    ...
    async fn passivate(&mut self) {
        ...
        loop {
            ...
            let resultado = self.channel.send((Command::Passivate(self.id), tx)).await;
            match resultado {
                Ok(_) => {},
                Err(e) => {
                    log::error!("Mandar Passivate con id: {} Ha fallado. Error: {}", self.id, e);
                    panic!("Mandar Passivate no deberia fallar"); 
                },
            };
    ...
    async fn activate(&mut self, c: u64) {
        ...
        loop {
            ...
            //     .ok()
            //     .unwrap();
            let resultado = self.channel.send((Command::Activate(c), tx)).await;
            match resultado {
                Ok(_) => {},
                Err(e) => {
                    log::error!("Mandar Activate a id: {} Ha fallado. Error: {}", c, e);
                    panic!("Mandar Activate no deberia fallar");
                },
            };
            result = rx.await.unwrap();
            if let Response::Breaker = result {
...
fn main() {
    let mut rt = tokio::runtime::Runtime::new().expect("Runtime deberia crearse correctamente");
    rt.block_on(async {
        env_logger::init();
